...
variable "boot_commands" {
  ...
  default = [
    "c",
    "<wait>",
    "linuxefi /images/pxeboot/vmlinuz inst.stage2=hd:LABEL=RHEL-9-7-0-BaseOS-x86_64 inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/radius.cfg quiet<enter>",
...
variable "post_install_script" {
  ...
  type    = string
  default = "Scripts/radius_setup.sh"
